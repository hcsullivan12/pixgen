...
class PixGenAnaTree : public art::EDAnalyzer 
{
public:
  explicit PixGenAnaTree(fhicl::ParameterSet const & p);
  virtual ~PixGenAnaTree();
...
//--------------------------------------------------------------------
PixGenAnaTree::PixGenAnaTree(fhicl::ParameterSet const & pset) 
 : EDAnalyzer{pset}
{
...
//--------------------------------------------------------------------
PixGenAnaTree::~PixGenAnaTree()
{}

//--------------------------------------------------------------------
void PixGenAnaTree::reconfigure(fhicl::ParameterSet const & pset)
{
...
//--------------------------------------------------------------------
void PixGenAnaTree::analyze(art::Event const & evt)
{
  // Reset variables
  ResetVars();
  ...
  if (!isdata)
  {
    ...
    if (tlist.size() > 1) throw cet::exception("PixGenAnaTree") << "MCTruth list greater than 1\n";
    if (glist.size() > 1) throw cet::exception("PixGenAnaTree") << "GTruth list greater than 1\n";
...
DEFINE_ART_MODULE(pixgen::PixGenAnaTree)
