...
PixGenGeometry::PixGenGeometry()
{}

//--------------------------------------------------------------------
PixGenGeometry::PixGenGeometry(fhicl::ParameterSet const& pset,
...
//--------------------------------------------------------------------
void PixGenGeometry::ValidateAndConfigure(
...
//--------------------------------------------------------------------
PixGenGeometry::Configuration_t
PixGenGeometry::ValidateConfiguration(
    fhicl::ParameterSet const& p,
    std::set<std::string> const& ignore_params /* = {} */) 
{
...
//--------------------------------------------------------------------
void PixGenGeometry::Configure(Configuration_t const& config) 
{
...
//--------------------------------------------------------------------
void PixGenGeometry::Initialize()
{
  ...
  {
    throw cet::exception("PixGenGeometry")
  ...
  if (fOpDetVolName.find("volOpDetSensitive") == std::string::npos) mf::LogWarning("PixGenGeometry") << "Couldn't find optical detector volume!\n";
  if (fLArTPCVolName.find("volLArActive") == std::string::npos)     throw cet::exception("PixGenGeometry") << "Couldn't find LAr active volume!\n";
  if (!fPixelPlane)                                                 throw cet::exception("PixGenGeometry") << "Couldn't find pixel plane volume!\n";

  // Load simplified geometry
  if (fUseSimpleGeometry) 
  {
    std::cout <<"\n";
    mf::LogInfo("PixGenGeometry")<<"Loading simple geometry\n";
    LoadSimpleGeometry();
  }
  std::cout << "\n";
  mf::LogInfo("PixGenGeometry")<<"Initialized geometry with " << fNPixels << " pixels\n";
}

//--------------------------------------------------------------------
void PixGenGeometry::LookAtNode(TGeoNode const* currentNode, std::string const& currentPath) 
{
...
//--------------------------------------------------------------------
void PixGenGeometry::LoadSimpleGeometry()
{
  fSimpleGeoY.clear();
  fSimpleGeoZ.clear();
...
//--------------------------------------------------------------------
void PixGenGeometry::GetOpDetCenter(double* xyz) const
{
  std::string planeNodePath = *std::find_if(fNodePaths.begin(), fNodePaths.end(), [](std::string const& s) {return s.find("PixelPlane") != std::string::npos;}); 
  gGeoManager->cd(planeNodePath.c_str());
  TGeoNode* node = gGeoManager->GetCurrentNode();
...
int PixGenGeometry::NearestPixelID(geo::Point_t const& point) const
{
...
//          decreasing y.
//
int PixGenGeometry::FindSimpleID(geo::Point_t const& point) const
{
  ...
  {
    throw cet::exception("PixGenGeometry") << "Further than 2 pixel spacings!\n";
  }
...
//--------------------------------------------------------------------
std::vector<float> PixGenGeometry::GetChannelPosition(const int& ch) const
{
...
//--------------------------------------------------------------------
std::string PixGenGeometry::VolumeName(geo::Point_t const& point) const
{
  ...
  if(std::abs(point.x()) > halfwidth  ||
    ...
    mf::LogWarning("PixGenGeometry") << "point (" << point.x() << ","
                                    << point.y() << "," << point.z() << ") "
