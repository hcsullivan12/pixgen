...
namespace pixgeng4 {
    ...
    if ( step->GetTotalEnergyDeposit() > 0 ){
      ...
      if( !fDontDriftThem ){
        ...
        if (bSingleTPC) {
          ...
          if (!pTouchable) {
            throw cet::exception
              ("PixGenG4") << "Untouchable step in LArVoxelReadout::ProcessHits()";
          }
	    ...
	    if (Has(fSkipWireSignalInTPCs, tpc))
	    {
          ...
          if (depth < pTouchable->GetHistoryDepth()) {
            ...
            throw cet::exception
              ("PixGenG4") << "No TPC ID found in LArVoxelReadout::ProcessHits()";
          } // if
                                                 ...
                                                 unsigned short int cryostat, unsigned short int tpc)
  {
	...
	XDrift = fGeoHandle->provider()->PlaneLocation(0)[0] - stepMidPoint.x()/CLHEP::cm;
      ...
      const int    nIonizedElectrons  = pixgeng4::IonizationAndScintillation::Instance()->NumberIonizationElectrons();
      const double energy             = pixgeng4::IonizationAndScintillation::Instance()->EnergyDeposit();
