...
// LArSoft Includes
#include "pixgen/LArG4/LArVoxelReadoutGeometry.h"
#include "pixgen/LArG4/PhysicsList.h"
#include "pixgen/LArG4/ParticleListAction.h"
#include "pixgen/LArG4/G4BadIdeaAction.h"
#include "pixgen/LArG4/IonizationAndScintillationAction.h"
#include "pixgen/LArG4/OpDetSensitiveDetector.h"
#include "pixgen/LArG4/OpDetReadoutGeometry.h"
#include "pixgen/LArG4/LArStackingAction.h"
#include "pixgen/LArG4/LArVoxelReadout.h"
#include "pixgen/LArG4/MaterialPropertyLoader.h"
#include "pixgen/LArG4/OpDetPhotonTable.h"
#include "pixgen/LArG4/AuxDetReadoutGeometry.h"
#include "pixgen/LArG4/AuxDetReadout.h"
...
namespace pixgeng4 {
   ...
   *   processed is specified explicitly in `PixGenG4` configuration
   * -# process all truth information generated so far: no generator is specified
   *   in the `PixGenG4` module configuration, and the module will process all
   ...
   * These particles are called, in `PixGenG4` jargon, _primaries_.
   *
   *
   * Output
   * -------
   *
   * The `PixGenG4` module produces:
   ...
   *   to Geant4. These particles are called, in `PixGenG4` jargon, _primaries_.
   *   The interface with Geant4 is via a helper class provided by _nutools_.
   ...
   *   (and, in particular, it's guaranteed to be unique within a `sim::PixGenG4`
   *   instance output).
   ...
   * -------
   *
   * The `PixGenG4` module produces `sim::SimChannel` objects from generated
   ...
   * -------------------------
   *
   * - *G4PhysListName* (string, default: `"pixgeng4::PhysicsList"`):
   ...
   *     `pixgeng4::LArVoxelReadout::SetOffPlaneChargeRecoveryMargin()`. A value of
   ...
   * -# a simplified one, implemented in `pixgeng4::OpBoundaryProcessSimple`
   *
   * The model is chosen according to the value of
   * `detinfo::DetectorProperties::SimpleBoundary()`, and the choice is
   * currently exerted by `pixgeng4::OpticalPhysics`.
   *
   ...
   *
   */
  class PixGenG4 : public art::EDProducer{
  public:

    /// Standard constructor and destructor for an FMWK module.
    explicit PixGenG4(fhicl::ParameterSet const& pset);
    virtual ~PixGenG4();
  ...
  private:
    ...
    pixgeng4::LArVoxelListAction* flarVoxelListAction; ///< Geant4 user action to accumulate LAr voxel information.
    pixgeng4::ParticleListAction* fparticleListAction; ///< Geant4 user action to particle information.
...
namespace pixgeng4 {
  ...
  //----------------------------------------------------------------------
  // Constructor
  PixGenG4::PixGenG4(fhicl::ParameterSet const& pset)
    : art::EDProducer{pset}
    , fG4Help                (0)
    , flarVoxelListAction    (0)
    , fparticleListAction    (0)
    , fG4PhysListName        (pset.get< std::string >("G4PhysListName","pixgeng4::PhysicsList"))
  ...
  {
    MF_LOG_DEBUG("PixGenG4") << "Debug: PixGenG4()";
    art::ServiceHandle<art::RandomNumberGenerator const> rng;

    if (pset.has_key("Seed")) {
      throw art::Exception(art::errors::Configuration)
        << "The configuration of PixGenG4 module has the discontinued 'Seed' parameter.\n"
  ...
  void PixGenG4::beginJob()
  {
    ...
    pixgeng4::MaterialPropertyLoader* MPL = new pixgeng4::MaterialPropertyLoader();
    ...
    //    pixgeng4::G4BadIdeaAction *bia = new pixgeng4::G4BadIdeaAction(fSmartStacking);
    ...
    //pixgeng4::IonizationAndScintillationAction *iasa = new pixgeng4::IonizationAndScintillationAction();
    //uaManager->AddAndAdoptAction(iasa);
    // User-action class for accumulating particles and trajectories
    // produced in the detector.
    fparticleListAction = new pixgeng4::ParticleListAction(lgp->ParticleKineticEnergyCut(),
  ...
  //----------------------------------------------------------------------
  void PixGenG4::beginRun(art::Run& run)
  {
  ...
  //----------------------------------------------------------------------
  std::unique_ptr<util::PositionInVolumeFilter> PixGenG4::CreateParticleVolumeFilter
    (std::set<std::string> const& vol_names) const
  {
  ...
  //----------------------------------------------------------------------
  void PixGenG4::produce(art::Event& evt)
  {
    MF_LOG_DEBUG("PixGenG4") << "produce()";
    ...
    for(size_t mcl = 0; mcl < mclists.size(); ++mcl){
      ...
      for(size_t m = 0; m < mclistHandle->size(); ++m){
        ...
        if (fdumpParticleList){
          mf::LogInfo("PixGenG4") << "Dump sim::ParticleList; size()="
                               << particleList.size() << "\n"
                               << particleList;
        }
    ...
    if (!lgp->NoElectronPropagation())
    {
      ...
      for (unsigned int c = 0; c < geom->Ncryostats(); ++c)
      {
        ...
        for (unsigned int t = 0; t < ntpcs; ++t)
        {
          ...
          if (!sd)
          {
            throw cet::exception("PixGenG4") << "Sensitive detector for cryostat "
          ...
          if (!larVoxelReadout)
          {
            throw cet::exception("PixGenG4") << "Sensitive detector '"
                                            << sd->GetName()
          ...
          if (!channels.empty())
          {
            MF_LOG_DEBUG("PixGenG4") << "now put " << channels.size() << " SimChannels"
                                                                        " from C="
    ...
    for(unsigned int a = 0; a < geom->NAuxDets(); ++a)
    {
      ...
      for(size_t sv = 0; sv < geom->NSensitiveVolume(); ++sv){
        ...
        pixgeng4::AuxDetReadout *auxDetReadout = dynamic_cast<pixgeng4::AuxDetReadout*>(sd);

        MF_LOG_DEBUG("PixGenG4") << "now put the AuxDetSimTracks in the event";
